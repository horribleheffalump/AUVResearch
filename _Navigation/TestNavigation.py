...
- Kalman filter for linear system with pseud measurements
...
[1] A. Miller, B. Miller, G. Miller, On AUV Control with the Aid of Position Estimation Algorithms Based on
Acoustic Seabed Sensing and DOA Measurements // Sensors 2019, 19(24), 5520
https://www.mdpi.com/1424-8220/19/24/5520
DOI: 10.3390/s19245520
[2]	A. Miller, B. Miller, G. Miller, AUV navigation with seabed acoustic sensing
// 2018 Australian & New Zealand Control Conference (ANZCC), Melbourne, Australia, 7-8 Dec. 2018
https://ieeexplore.ieee.org/document/8606561 
DOI: 10.1109/ANZCC.2018.8606561
[3] A. Miller, B. Miller, G. Miller, AUV position estimation via acoustic seabed profile measurements
// 2018 IEEE/OES Autonomous Underwater Vehicle Symposium - AUV 2018, University of Porto, Porto, Portugal, 6-9 Nov. 2018
...
# constant AUV speed absolute value for nominal (desired) path calculation
v = 2.0  # 5.0
# controls' sequences for the nominal path calculation
...
# array of speed vectors at time instants t_history for the nominal path
VNominal_history = np.array(list(map(lambda t: AUV.V(U(t)), t_history[:-1])))
# array of position increments at time instants t_history for the nominal path
...
# trigonometrical functions for pseudo measurements recalculation
def tan2sin(tan):
...
def Psi1(auv, k, X, y):
    ...
    tanlambda = (X[2] - Xb[:, 2]) / np.sqrt(
        (X[0] - Xb[:, 0]) * (X[0] - Xb[:, 0]) + (X[1] - Xb[:, 1]) * (X[1] - Xb[:, 1]))
...
def dPsi1(auv, k, X, y):
    ...
    d_tanlambda_dX = -(X[2] - Xb[:, 2]) * (X[0] - Xb[:, 0]) / np.power(
        np.power(X[0] - Xb[:, 0], 2) + np.power(X[1] - Xb[:, 1], 2), 1.5)
    d_tanlambda_dY = -(X[2] - Xb[:, 2]) * (X[1] - Xb[:, 1]) / np.power(
        np.power(X[0] - Xb[:, 0], 2) + np.power(X[1] - Xb[:, 1], 2), 1.5)
...
def Psi1Pseudo(auv, k, X, y):
    ...
    return np.vstack((np.array([-sin_phi, cos_phi, np.zeros(sin_phi.shape[0])]).T,
                      np.array([-sin_lambda, np.zeros(sin_phi.shape[0]), cos_phi * cos_lambda]).T)) @ X
...
def dPsi1Pseudo(auv, k, X, y):
    ...
    sin_phi, cos_phi, sin_lambda, cos_lambda = obs2sincos(y, X)
    return np.vstack((np.array([-sin_phi, cos_phi, np.zeros(sin_phi.shape[0])]).T,
                      np.array([-sin_lambda, np.zeros(sin_phi.shape[0]), cos_phi * cos_lambda]).T))
...
kalman = KalmanFilter(Phi1, dPhi1, Phi2, Psi1, dPsi1, Psi2, np.array([0.0, 0.0, 0.0]), np.diag(DW),
                      np.zeros(2 * Xb.shape[0]), np.diag(DNu))

# pseudo measurements Kalman filter definition

pseudo = KalmanFilter(Phi1, dPhi1, Phi2, Psi1Pseudo, dPsi1Pseudo, Psi2Pseudo, np.array([0.0, 0.0, 0.0]), np.diag(DW),
                      np.zeros(2 * Xb.shape[0]), np.diag(DNu))
...
# set of filters for position estimation, their names and do they need the pseudo-measurements
    ...
    for k in range(0, len(filters)):
        ...
        for i in range(0, N):
            ...
            if needsPseudoMeasurements[k]:
                # if the filter requires pseudo-measurements
        ...
        np.savetxt(ControlErrorFileNameTemplate.replace('[filter]', names[k]).replace('[pathnum]',
                                                                                      str(m).zfill(int(np.log10(M)))),
                   ControlError[k][m, :, :], fmt='%f')
...
for k in range(0, len(filters)):
    ...
    np.savetxt(ControlErrorFileNameTemplate.replace('[filter]', names[k]).replace('[pathnum]', 'mean'),
               mControlError[k], fmt='%f')
    np.savetxt(ControlErrorFileNameTemplate.replace('[filter]', names[k]).replace('[pathnum]', 'std'),
               stdControlError[k], fmt='%f')
