...
#filename_template_estimate_error = "D:\\Наука\\_Статьи\\__в работе\\2019 - Sensors - Navigation\\data\\acoustic\\estimate\\estimate_error_[filter]_[pathnum].txt"
...
for k in range(0, len(filters)):
    mControlError[k] = np.loadtxt(ControlErrorFileNameTemplate.replace('[filter]',filters[k]).replace('[pathnum]', 'mean'))
    stdControlError[k] = np.loadtxt(ControlErrorFileNameTemplate.replace('[filter]',filters[k]).replace('[pathnum]', 'std'))
