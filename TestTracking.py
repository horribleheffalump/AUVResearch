...
def Phi(model, k, X, XHat):
    ...
    return np.hstack((x_orig, v_orig, X_turned + deltaX, X[14:19])) # original coordinates, coordinates in turned plane, alpha, beta, R0
...
def toOriginalCoordinates(X, alpha, beta, R0):
    X_ = X[0:3]
    V_ = X[3:6]
    A = np.array([[np.cos(beta), np.sin(alpha) * np.sin(beta), np.sin(beta) *np.cos(alpha)],[1.0, np.cos(alpha), -np.sin(alpha)],[-np.sin(beta), np.sin(alpha) * np.cos(beta), np.cos(beta) * np.cos(alpha)]])
    X = A.T @ X_ + R0
    V = A.T @ V_
    return X,V
...
omega0 = 20.0 #[Hz] frequensy of sound signal
С = 1500.0 #[m/s] sound speed
...
def Psi(model, k, X, y):
    ...
    x = X[0:3]
    v = X[3:6]
    R = np.sqrt((x[0] - Xb[:,0]) * (x[0] - Xb[:,0]) + (x[1] - Xb[:,1]) * (x[1] - Xb[:,1]) + (x[2] - Xb[:,2]) * (x[2] - Xb[:,2]))
...
sigmaX0 = np.array([1e-2, 1e-2, 1e-2, 1e-2, 1e-2, 1e-2, 1e2, 1e2, 1e2, 2.0, 2.0, 2.0, 1e-1, 1e-1, np.pi/50.0, np.pi/50.0, 1000, 1000, 100])
...
X0all = np.array(list(map(lambda i: mX0 + sigmaX0 * np.array(np.random.normal(0,1,sigmaX0.shape[0])), range(0, Mtrain) ))) # initial point for the training sample paths
models = np.array(list(map(lambda i: PseudoAUV(), range(0, Mtrain) )))  # models for compartibility with CMNF
